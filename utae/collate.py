...
def pad_collate(batch, pad_value=0):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if torch.utils.data.get_worker_info() is not None:
            ...
            # shared memory tensor to avoid an extra copy. Pre-shape the buffer to
            # the stacked shape so torch.stack(out=...) doesn't resize it (which
            # raises a deprecation UserWarning on newer PyTorch).
            numel = sum([x.numel() for x in batch])
            storage = elem.untyped_storage()._new_shared(numel * elem.element_size())
            out = elem.new(storage).resize_(len(batch), *batch[0].shape)
